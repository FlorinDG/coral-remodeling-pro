...
const PREVIEW_TOTAL = 2500 + 35 * 45 + 16 * 65 + 45 * 28 + 12 * 32;
...
export default function DocumentTemplatesModule() {
    ...
    const [stationeryUrl, setStationeryUrl] = useState<string | null>(null);
    ...
    const handleStationeryUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const reader = new FileReader();
        reader.onloadend = () => {
            setStationeryUrl(reader.result as string);
            setStationeryName(file.name);
            toast.success('Stationery loaded — click "Save Branding" to persist.');
        };
        reader.readAsDataURL(file);
    };

    const handleRemoveStationery = () => {
        setStationeryUrl(null);
            ...
            {mode === 'dynamic' ? (
                ...
                <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    <div className="flex flex-col">
                        ...
                        <p className="text-sm text-neutral-500">Upload a full A4 letterhead image (PNG or JPG). Your data will be overlaid directly onto this background — no template headers or footers will be rendered.</p>
                    </div>
                        ...
                        <div className="w-full max-w-2xl mx-auto rounded-xl border-2 overflow-hidden relative" style={{ borderColor: `${primaryColor}80` }}>
                            ...
                            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-4">
                                <div className="flex items-center gap-2">
                                    ...
                                    <div>
                                        <p className="text-sm font-bold text-white">Stationery ready</p>
                                        {stationeryName && <p className="text-xs text-white/70">{stationeryName}</p>}
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {/* Brand color — still configurable in stationery mode for table accents */}
                    <div className="max-w-2xl mx-auto w-full">
                        <div className="flex items-center justify-between p-4 rounded-lg border border-neutral-200 dark:border-white/10 bg-white dark:bg-[#111]">
                            <div className="flex flex-col gap-1">
                                <span className="text-sm font-medium text-neutral-900 dark:text-white">Accent Color</span>
