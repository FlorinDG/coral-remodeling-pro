        ...
        <div className="bg-neutral-50 dark:bg-white/5 rounded-[2.5rem] border border-neutral-200 dark:border-white/5 p-8 h-full flex flex-col shadow-sm">
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-2xl font-bold flex items-center gap-3 text-neutral-900 dark:text-white">
                    {t('tasks')}
                    <span className="bg-[#d35400] text-white text-[10px] font-black px-2 py-0.5 rounded-full">{remainingTasks}</span>
                </h3>
                {!readOnly && !isAdding && (
            ...
            <div className="flex-1 overflow-y-auto space-y-4 mb-2 pr-2 custom-scrollbar">
                {isAdding && (
                    <div className="glass-morphism p-6 rounded-3xl border border-[#d35400]/20 animate-in slide-in-from-top-4 duration-300">
                        <div className="flex justify-between items-center mb-4">
                            <span className="text-[10px] font-black uppercase tracking-widest text-[#d35400]">New Task</span>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="space-y-1">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-white dark:bg-black/40 border border-neutral-200 dark:border-white/10 rounded-xl px-4 py-2 text-xs focus:border-[#d35400] outline-none transition-colors"
                                    />
                                </div>
                                <div className="space-y-1">
                ...
                {tasks.map(task => (
                    <div
                        ...
                        className={`p-5 rounded-[1.5rem] border transition-all flex flex-col gap-3 ${task.status === 'DONE'
                            ...
                            : 'bg-white dark:bg-black/40 border-neutral-200 dark:border-white/10 hover:border-[#d35400]/50 shadow-sm'
                            }`}
                    >
                        <div className="flex items-center gap-4">
                            <div
                                onClick={() => toggleStatus(task.id, task.status)}
                                className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 cursor-pointer transition-colors ${task.status === 'DONE' ? 'bg-[#d35400] border-[#d35400]' : 'border-neutral-300 dark:border-neutral-700'
                                    }`}>
                            ...
                            <div className="flex gap-4 ml-10">
                                ...
                                {task.fileUrl && (
                                    <a
                                        href={task.fileUrl}
                                        target="_blank"
                                        className="flex items-center gap-1.5 text-[10px] font-bold text-[#d35400] uppercase tracking-widest bg-[#d35400]/10 px-2 py-1 rounded-lg hover:bg-[#d35400]/20 transition-colors"
                                    >
