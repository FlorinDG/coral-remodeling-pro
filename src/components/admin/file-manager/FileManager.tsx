    ...
    if (nodes.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-12 text-neutral-500">
                ...
                <p>No files found.</p>
                <p className="text-sm opacity-60">Upload files to get started.</p>
            </div>
        );
    }
...
interface FileManagerProps {
    ...
    contextId?: string;
    driveFolderId?: string; // Optional direct mapping to a Google Drive folder (legacy, to be removed)
}
...
export default function FileManager({ contextType, contextId }: FileManagerProps) {
    ...
    const scopedNodes = useMemo(() => {
        if (isGlobalMode) return nodes;
        return nodes.filter(n => n.contextType === contextType && n.contextId === contextId);
    }, [nodes, isGlobalMode, contextType, contextId]);

    // 2. Filter nodes based on Tag Filter OR Search Query
    const displayedNodes = useMemo(() => {
        let filtered = scopedNodes;
        if (tagFilter && isGlobalMode) {
            filtered = filtered.filter(n => n.contextType === tagFilter);
        }
        if (searchQuery.trim()) {
            filtered = filtered.filter(n => n.name.toLowerCase().includes(searchQuery.toLowerCase()));
        }
        return filtered;
    }, [scopedNodes, searchQuery, tagFilter, isGlobalMode]);

    // Grouping logic for Global Mode
    const groupedNodes = useMemo(() => {
        if (!isGlobalMode) return { 'Files': displayedNodes };

        const groups: Record<string, FileNode[]> = {};
        displayedNodes.forEach(node => {
            const ctx = node.contextType || 'global';
            if (!groups[ctx]) groups[ctx] = [];
            groups[ctx].push(node);
        });
        return groups;
    }, [displayedNodes, isGlobalMode]);

    const contextTypeLabels: Record<string, { label: string, icon: React.ReactNode }> = {
        'project': { label: 'Projects', icon: <PackageOpen className="w-4 h-4 mr-1.5" /> },
        'quotation': { label: 'Quotations', icon: <FileSignature className="w-4 h-4 mr-1.5" /> },
        'invoice': { label: 'Invoices', icon: <Receipt className="w-4 h-4 mr-1.5" /> },
        'receipt': { label: 'Receipts', icon: <Receipt className="w-4 h-4 mr-1.5" /> },
        'global': { label: 'Global / Uncategorized', icon: <LayoutGrid className="w-4 h-4 mr-1.5" /> }
    };
    ...
    const breadcrumbs = useMemo(() => {
        ...
        return [{ id: 'root', name: isGlobalMode ? 'Global Library' : 'Files' }];
    }, [isGlobalMode, searchQuery, tagFilter]);
        ...
        <div className="flex w-full h-full bg-background rounded-b-xl overflow-hidden">
            <div className="flex-1 flex flex-col min-w-0 bg-background border-l border-border/50">
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-xs md:text-sm">
                        ...
                        {isGlobalMode && (
                            <div className="hidden lg:flex items-center gap-1.5 border-l border-border/50 pl-3 h-5">
                                {['project', 'quotation', 'invoice', 'receipt'].map(tag => (
                                    <button
                                        key={tag}
                                        onClick={() => {
                                            const newTag = tagFilter === tag ? null : tag as FileContextType;
                                            setTagFilter(newTag);
                                            if (newTag) {
                                                setSearchQuery('');
                                            }
                                        ...
                                        className={cn(
                                            "px-2.5 py-0.5 rounded-full text-[10px] font-medium transition-colors border whitespace-nowrap capitalize",
                                            tagFilter === tag
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar bg-neutral-50/30 dark:bg-black relative">
                    ...
                    <div className="pb-8">
                        {Object.entries(groupedNodes).map(([ctxType, nodesInGroup]) => (
                            <div key={ctxType} className="mb-6">
                                {isGlobalMode && (
                                    <div className="px-4 pt-4 pb-2 flex items-center text-sm font-semibold text-muted-foreground">
                                        {contextTypeLabels[ctxType]?.icon || <FileIcon className="w-4 h-4 mr-1.5" />}
                                        {contextTypeLabels[ctxType]?.label || (ctxType.charAt(0).toUpperCase() + ctxType.slice(1) + 's')}
                                        <span className="ml-2 text-xs bg-muted px-2 py-0.5 rounded-full">{nodesInGroup.length}</span>
                                    </div>
                                )}
                                <FileDisplayArea
                                    nodes={nodesInGroup}
                        ...
                        {Object.keys(groupedNodes).length === 0 && !isLoading && (
