...
function TimesheetsContent() {
    ...
    const handleBulkAction = async (action: 'approved' | 'denied') => {
        ...
        if (!window.confirm(`${action === 'approved' ? 'Approve' : 'Deny'} ${selectedEntries.size} entries, totalling ${selectedHours} hours?`)) return;
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (!currentParams.has('from') || !currentParams.has('to')) {
                const now = new Date();
                currentParams.set('from', formatISO(startOfWeek(now, { weekStartsOn: 1 })));
                currentParams.set('to', formatISO(endOfWeek(now, { weekStartsOn: 1 })));
            }
            ...
            <div className="flex flex-col gap-6 p-6">
                ...
                {summary && !error && (
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-2">
                        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-white/10 rounded-2xl p-4 shadow-sm flex flex-col justify-between">
                            <h3 className="text-xs font-bold text-neutral-500 uppercase tracking-wider">{t('totalHours', { fallback: 'Totaal uren' })}</h3>
                            <p className="text-2xl font-black mt-2">{summary.totalHours}u</p>
                        </div>
                        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-white/10 rounded-2xl p-4 shadow-sm flex flex-col justify-between">
                            <h3 className="text-xs font-bold text-neutral-500 uppercase tracking-wider">{t('billableInternal', { fallback: 'Factureerbaar / Intern' })}</h3>
                            <p className="text-xl font-black mt-2">{summary.billableHours}u <span className="text-neutral-400 font-normal">/ {summary.internalHours}u</span></p>
                        </div>
                        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-white/10 rounded-2xl p-4 shadow-sm flex flex-col justify-between">
                            <h3 className="text-xs font-bold text-neutral-500 uppercase tracking-wider">{t('approvedPending', { fallback: 'Goedgekeurd / Te beoordelen' })}</h3>
                            <p className="text-xl font-black mt-2 text-green-600">{summary.approvedHours}u <span className="text-orange-500 font-normal">/ {summary.pendingHours}u</span></p>
                        </div>
                        <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-white/10 rounded-2xl p-4 shadow-sm flex flex-col justify-between">
                            <h3 className="text-xs font-bold text-neutral-500 uppercase tracking-wider">{t('unattributedHours', { fallback: 'Niet toegewezen uren' })}</h3>
                ...
                <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-white/10 rounded-2xl overflow-hidden shadow-sm mt-4">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-neutral-200/50 dark:divide-white/5">
                            {groupBy === 'flat' && entries.map((entry: any) => {
                                    ...
                                    <tr key={entry.id} className="hover:bg-neutral-50/50 dark:hover:bg-white/5 transition-colors group">
                                        ...
                                        <td className="px-6 py-4">
                                            ...
                                            <div className="text-xs text-neutral-500 flex gap-1 flex-wrap mt-1">
                                                ...
                                                {entry.source === 'manual' && <span className="bg-purple-100 text-purple-700 px-1.5 rounded-sm uppercase tracking-widest" style={{ fontSize: '9px' }}>Manual</span>}
                                                {!entry.clockOutTime && <span className="bg-red-100 text-red-700 px-1.5 rounded-sm uppercase tracking-widest" style={{ fontSize: '9px' }}>No Clock-out</span>}
                                                {entry.source === 'adjusted' && <span className="bg-blue-100 text-blue-700 px-1.5 rounded-sm uppercase tracking-widest" style={{ fontSize: '9px' }}>Adjusted</span>}
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                            ...
                            {groupBy === 'worker' && summary && summary.rollups?.byWorker.map((worker: any) => (
                                <tr key={worker.userId} className="bg-neutral-50 dark:bg-neutral-800 border-b border-neutral-200 dark:border-white/10">
                                    <td colSpan={7} className="px-6 py-4 text-sm">
                                        <div className="flex items-center justify-between">
                                            <span className="font-bold">{worker.workerName}</span>
                            ...
                            {groupBy === 'project' && summary && summary.rollups?.byProject.map((project: any) => (
                                <tr key={project.projectId} className="bg-neutral-50 dark:bg-neutral-800 border-b border-neutral-200 dark:border-white/10">
                                    <td colSpan={7} className="px-6 py-4 text-sm">
                                        <div className="flex items-center justify-between">
