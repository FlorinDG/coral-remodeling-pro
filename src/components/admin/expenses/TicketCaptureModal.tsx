        ...
        try {
            ...
            if (isInvoiceMode) {
                setForm(prev => ({
                    ...
                    ogm: ext.structuredCommunication || prev.ogm,
                    object: ext.subject || prev.object,
        ...
        try {
            ...
            if (isInvoiceMode) {
                updatedProps = {
                    ...
                    supplierBic: scanResult?.extracted?.supplierBic || currentProps.supplierBic,
                    reverseCharge: scanResult?.extracted?.reverseCharge || currentProps.reverseCharge,
                    vatBreakdown: scanResult?.extracted?.vatBreakdown || currentProps.vatBreakdown,
                    supplierAddress: scanResult?.extracted?.supplierAddress ? JSON.stringify(scanResult.extracted.supplierAddress) : currentProps.supplierAddress,
                    totalExVat: parsedAmount,
                    totalVat: parsedVat,
                    totalIncVat: parsedAmount + parsedVat,
                    status: 'opt-unpaid',
