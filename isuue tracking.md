...
33. [PARTIAL] Custom selects: Rolled out `SearchableSelect` replacement in Team Settings (access level), Company Info (interface language, document language, connector, date format, number width), UI Settings (module selector), Calendar (calendar selector, portal selector, task priority), and Email Sidebar (account switcher). Total: 14 native selects replaced across 5 high-traffic pages. Remaining: financial engines, filter/sort toolbars, database schema editor, and low-traffic pages.
34. [PARTIAL] Task board editable titles + action bar: Added `EditableTitle` component to `TaskBoardView.tsx`. Users can now click a task card title to edit inline — Enter/blur saves, Escape cancels. Added action bar under title with: (1) priority pill that cycles P1→P2→P3→P4 on click, (2) due date control with calendar icon and hidden date picker, (3) block/attachment count indicator. All wired via `TaskModuleShell` callbacks.
