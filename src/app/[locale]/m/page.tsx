...
export default async function MobileDashboard({ params }: { params: Promise<{ locale: string }> }) {
    ...
    return (
        <div className="max-w-lg mx-auto flex flex-col gap-0 min-h-[calc(100vh-8rem)]">
            ...
            <div className="px-4 pt-5 pb-4 space-y-3">
                ...
                <div className="flex items-start justify-between">
                    ...
                    <div className="text-right mt-1">
                        ...
                        <p className="text-[12px] font-black uppercase tracking-widest text-neutral-800 dark:text-neutral-200">
                ...
                <div className="grid grid-cols-3 gap-2">
                    <div className="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-300 dark:border-white/10 p-2.5 text-center shadow-sm">
                        ...
                        <p className="text-[12px] text-neutral-800 dark:text-neutral-200 font-extrabold uppercase tracking-wide mt-0.5">{t('nav_invoices')}</p>
                    </div>
                    <div className="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-300 dark:border-white/10 p-2.5 text-center shadow-sm">
                        <p className="text-lg font-black text-orange-600 dark:text-orange-400">{draftCount}</p>
                        <p className="text-[12px] text-neutral-800 dark:text-neutral-200 font-extrabold uppercase tracking-wide mt-0.5">{t('dash_drafts')}</p>
                    </div>
                    <div className="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-300 dark:border-white/10 p-2.5 text-center shadow-sm">
                        <p className="text-lg font-black text-neutral-950 dark:text-neutral-100">{totalExpenses}</p>
                        <p className="text-[12px] text-neutral-800 dark:text-neutral-200 font-extrabold uppercase tracking-wide mt-0.5">{t('nav_expenses')}</p>
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-300 dark:border-white/10 p-3 flex items-center justify-between shadow-sm">
                    <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[12px] text-neutral-800 dark:text-neutral-200 font-bold">{t('dash_in')}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[12px] text-neutral-800 dark:text-neutral-200 font-bold">{t('dash_out')}</span>
                    </div>
                    {unpaidCount > 0 && (
                        <>
                            <div className="w-px h-4 bg-neutral-300 dark:bg-white/20" />
                            <div className="flex items-center gap-1">
                                ...
                                <span className="text-[12px] text-blue-700 dark:text-blue-400 font-extrabold">{unpaidCount} {t('dash_awaiting_payment')}</span>
                            </div>
                        </>
                    )}
                </div>
            ...
            {/* ═══════════════════════════════════════════════════════
                MIDDLE THIRD — Quota Indicator + Primary Actions
            ═══════════════════════════════════════════════════════ */}
            <div className="px-4 pb-4 flex flex-col gap-3">
                {/* Scan quota indicator (moved above buttons, informational status only) */}
                <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-300 dark:border-white/10 p-3.5 flex items-center justify-between shadow-sm">
                    <div className="flex items-center gap-2.5">
                        <Camera className="w-4.5 h-4.5 text-orange-600 dark:text-orange-400" />
                        <span className="text-xs font-extrabold text-neutral-950 dark:text-neutral-205">{t('exp_ocr_scans')}</span>
                    </div>
                    <div className="flex items-center gap-2.5">
                        <div className="w-24 h-2 rounded-full bg-neutral-200 dark:bg-white/10 overflow-hidden border border-neutral-300 dark:border-white/5">
                            <div
                                className="h-full rounded-full bg-orange-500 transition-all"
                                style={{ width: `${Math.min(100, (scansUsed / scanQuota) * 100)}%` }}
                            />
                        </div>
                        <span className="text-[12px] font-black text-neutral-800 dark:text-neutral-200">{scansUsed}/{scanQuota}</span>
                    </div>
                </div>
                ...
                <Link
                    href="/m/invoices/new"
                    className="flex items-center justify-between gap-3 px-4 py-4 rounded-2xl text-white font-bold shadow-lg active:scale-[0.98] transition-all border border-transparent hover:border-white/20 min-h-[52px]"
                ...
                >
                    <div className="flex items-center gap-3.5">
                        ...
                        <span className="text-[15px] font-black leading-tight">{t('dash_create_invoice')}</span>
                    </div>
                ...
                <Link
                    href="/m/expenses"
                    className="flex items-center justify-between gap-3 px-4 py-4 rounded-2xl bg-neutral-950 dark:bg-neutral-900 text-white font-bold shadow-lg active:scale-[0.98] transition-all border border-neutral-850 dark:border-white/10 min-h-[52px]"
                >
                    <div className="flex items-center gap-3.5">
                        ...
                        <div className="flex flex-col">
                            <span className="text-[15px] font-black leading-tight">{t('dash_add_expense')}</span>
                            <span className="text-[12px] opacity-85 font-bold mt-0.5">{scansUsed}/{scanQuota} {t('dash_scans')}</span>
                        </div>
                    </div>
                    <ChevronRight className="w-5 h-5 opacity-80" />
                </Link>
                ...
                <Link
                    href="/m/clients"
                    className="flex items-center justify-between gap-3 px-4 py-4 rounded-2xl bg-blue-700 text-white font-bold shadow-lg active:scale-[0.98] transition-all border border-transparent hover:border-white/20 min-h-[52px]"
                >
                    <div className="flex items-center gap-3.5">
                        <div className="w-11 h-11 rounded-xl bg-white/20 flex items-center justify-center shrink-0">
