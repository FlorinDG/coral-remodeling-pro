...
- **GRID-1** 🩹 Cell editing is janky: many clicks to enter edit, cell flickers and drops out of edit mode. Fix the core edit interaction. (highest-leverage UX fix)
...
- **SCHEMA-1b** 🩸 DATA-SAFE merge of EXISTING duplicates (AFTER 1a verified). **Dry-run first.**
...
## FILES — Storage (v0.2/0.4)
- **FILES-1** 🩹 Files module creates a folder for EVERY new ERP item regardless of type/location — not intended. Research a proper file-explorer model; Planner to recommend coder instructions. (supersedes F4's scattered-folder issue + the CoralDrive nesting)
- **FILES-2** Tenant folders nest inside the `CoralDrive` folder (not Drive root); relocate scattered roots. (Florin to supply CoralDrive folder ID.) (was F4)
- **FILES-3** "Saved but not findable" — saved files must appear in BOTH the in-app explorer and the real Drive path.
