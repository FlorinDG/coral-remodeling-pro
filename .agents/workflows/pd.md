...
5b. **`_prisma_migrations` IS NOT EVIDENCE ABOUT THE DATABASE.** This repo's tables predate its migration history (the `db push` era), so the record and the reality drift **in both directions**. Observed 2026-09-24, three times in one session: a FK present in Postgres but undeclared in `schema.prisma`; a migration recorded unapplied whose columns already existed; a constraint the history never mentioned.
   - 🛑 **Never state what a database contains from a migration file, a migration record, or the absence of one.** Query the database — `pg_constraint`, `information_schema.columns`, or `migrate diff --from-schema-datasource --to-schema-datamodel`.
   - 🛑 **Never state what a database contains from CODE SHAPE either** — "there is no cascade and no cleanup code, therefore orphans" was wrong because Postgres had been enforcing a FK nobody had declared.
   - **The authority is the database. Everything else is a claim about it.**
...
7. **Files, attachments, photos, documents** served through the authenticated route with a tenant check — never a bare blob key or public URL. *(Past: `RECEIPT-BONNETJE-LINK`.)* **`lib/storage/**` is the ONLY module permitted to import `@vercel/blob`** (with one documented exception: `app/api/files/[...key]/route.ts` which streams directly without buffering). All other modules, server actions, and routes MUST use the `StorageProvider` abstraction (`storage`).
