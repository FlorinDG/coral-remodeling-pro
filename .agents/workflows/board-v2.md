...
- **FIN-2** 🟢→ CN numbering ✅ code-confirmed + empirically verified (CN-2026-006 rendered correct). `DocType` now incl. `'creditnote'` with own DB-derived sequence (next-document-number.ts:126). ⏳ Florin still verify the INVOICE-import-no-skip half (import keeps own number, counter doesn't advance). (was B4 + CN numbering)
...
- **FILES-0** Define `StorageProvider` interface in `src/lib/storage/index.ts`: `put(key, data, opts) → {key, url}`, `get(key) → stream/url`, `delete(key)`, `list(prefix) → entries`. Add `BlobStorageProvider` (`@vercel/blob`) as the implementation. **Key scheme (tenant-isolated, flat):** `t_{tenantId}/{recordType}/{recordId}/{filename}` (e.g. `t_ten-gar163.../receipt/pg-abc/aldi-2026-06.pdf`). No folder objects — the prefix IS the namespace. **EXCEPTION — projects allow subfolders:** project keys may carry an arbitrary nested subpath: `t_{tenantId}/project/{recordId}/{subpath…}/{filename}` (e.g. `…/project/pg-7/permits/2026/bouwvergunning.pdf`). Blob keys are plain slash-delimited strings, so subfolders cost nothing at storage level — they are VIRTUAL, derived from key segments, never stored folder objects (so the "folder-per-item" bug still can't recur). Env: `BLOB_READ_WRITE_TOKEN` (Florin provisions in Vercel). **This task ships the seam with NO behavior change yet — provider exists, nothing calls it.**
